...
export const ModalEditarCuentaRole = ({ isOpen, closeModal, obtenerId }) => {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = handleSubmit(async (data) => {
    const res = await client.put(`/users-role/${obtenerId}`, data);
              ...
              <div className="inline-block w-1/3 max-md:w-full p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
                ...
                >
                  <div className="flex flex-col gap-2 max-md:text-sm">
                    <label htmlFor="">SELECCIONAR PARA DESACTIVAR</label>
                    <select
                      {...register("role_id", { required: true })}
                      type="text"
                      className="text-slate-700 bg-white rounded-xl py-2.5 px-2 border-slate-300 border-[1px] uppercase text-sm"
                    >
                      <option value="">Seleccionar</option>
                      <option value={1}>Si autorizado</option>
                      <option value={2}>Desautorizar</option>
                    </select>
                  </div>
