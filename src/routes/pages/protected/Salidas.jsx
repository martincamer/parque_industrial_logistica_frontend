...
export const Salidas = () => {
  ...
  const totalContratosMetrosCuadrados = filteredData?.reduce(
    (total, salida) => {
      const metrosCuadradosArray = salida?.datos_cliente?.datosCliente;
      if (metrosCuadradosArray && metrosCuadradosArray.length > 0) {
        // Assuming you want to sum up the metrosCuadrados for all items in the array
        const metrosCuadradosSum = metrosCuadradosArray.reduce(
          (sum, cliente) => {
            return sum + parseFloat(cliente.metrosCuadrados || 0);
          },
          0
        );
        return total + metrosCuadradosSum;
      }
      return total;
    },
    0
  ...
  return (
    <section className="w-full h-full min-h-screen max-h-full">
      <div className="bg-gray-100 py-10 px-10 flex justify-between items-center max-md:flex-col max-md:gap-3">
        ...
        >
          Cargar nueva salida de contratos
        </button>
      </div>

      <div className="px-5 pt-10 grid grid-cols-4 gap-2 max-md:grid-cols-1 max-md:pt-5 max-md:pb-4">
        <div className="bg-gray-800 py-5 px-10 rounded-xl shadow">
          <div className="flex flex-col gap-1 items-center">
            ...
            <p className="text-white font-medium text-xl">
              {formatearDinero(totalEnSalidasAnual)}
            </p>
          </div>
        </div>{" "}
        <div className="bg-gray-800 py-5 px-10 rounded-xl shadow">
        ...
        <div className="bg-gray-800 py-5 px-10 rounded-xl shadow">
          <div className="flex flex-col gap-1 items-center">
            ...
            <p className="text-white font-medium text-xl">
              {totalContratosMetrosCuadrados.toFixed(2)} mtrs.
            </p>
          </div>
        </div>
      </div>

      <div className="flex gap-3 mx-5 justify-between mt-10 mb-2 max-md:mt-3 w-auto max-md:justify-center">
        <div className="flex items-center gap-3 max-md:flex-col max-md:items-stretch w-auto">
          <div className="flex gap-2 max-md:flex-col w-auto">
                  ...
                  <td className="uppercase">
                    <div className="flex gap-1">
                      {s?.datos_cliente?.datosCliente?.map((p) => (
                        <p className="font-bold border rounded-md border-gray-300 py-1 px-4">
...
const ModalCrearSalida = () => {
  ...
  const handleAddClient = () => {
    ...
    if (isNaN(Number(nuevoCliente.metrosCuadrados))) {
      setError("Los campos 'Metros Cuadrados' deben ser numéricos.");
      return;
    }
        ...
        <form className="flex flex-col gap-4">
          <div className="flex gap-2 max-md:gap-2 my-4">
            <button
              ...
              className="bg-gradient-to-r from-gray-800 to-gray-600/90 px-4 text-white rounded-md font-bold text-sm py-1.5"
            >
              Crear choferes
            </button>
            <button
              ...
              c
              className="bg-gradient-to-r from-yellow-500 to-primary px-4 text-white rounded-md font-bold text-sm py-1.5"
            >
    ...
    <dialog id="my_modal_actualizar_salida" className="modal">
      <div className="modal-box rounded-md max-w-full h-full scroll-bar max-md:w-full max-md:max-h-full max-md:rounded-none">
        ...
        <form className="flex flex-col gap-4">
          ...
          <article>
            <div className="flex flex-col gap-3 max-md:gap-5 mt-5">
              <h3 className="font-bold max-md:text-sm text-gray-800 text-base">
                Seleccionar Fabrica de Salida y Localidad.
              </h3>
              <div className="grid grid-cols-4 items-center gap-2 max-md:grid-cols-1 max-md:gap-5">
